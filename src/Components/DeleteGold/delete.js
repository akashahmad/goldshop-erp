...
import Style from '../DeletePopup/style'
import axios from 'axios'
import {apiPath} from '../../Config'
import {withRouter} from 'react-router-dom'

const Table = (props) => {
    let { selectedId, setSelectedId, deleteModel, setDeleteModel ,  setViewGold} = props;
    const HandleRemoveItem = () => {
        axios.delete(apiPath + "/api/gold/" + selectedId).then(res => {
            setSelectedId("");
            setViewGold(customers => customers.filter(item => item.id !== selectedId));
                ...
                <div class="modal-content d-flex model-header">
                    ...
                    <div className="model-body-of-deletemethod">
                        <div className="option-text-delete-model fnt-poppins">
                            <h6 className="mr-5">Are you sure you want to delete this record</h6>
                        </div>
                    </div>
